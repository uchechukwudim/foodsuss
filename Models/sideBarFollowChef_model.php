...
/**
 * Description of homeFollowChef_model
 *
 * @author Uche
 */
class SideBarFollowChef_model extends Model {
    //put your code here
    ...
    function followChef()
    {
        ...
        $chefsFollow = $sth->fetchAll();
          $recipePostCount = $this->recipePostCount($chefsFollow, $this->id);
         $friendsCount = $this->friendsCount($chefsFollow, $this->id);
         $followerCount = $this->followerCount($chefsFollow, $this->id);
         echo json_encode($this->loadChefFollows($chefsFollow, $this->id, $recipePostCount,  $friendsCount,  $followerCount, $page=''));
    }
    
    private function loadChefFollows($chefsFollow, $userId, $recipePostCount,  $friendsCount,  $followerCount, $page)
    {
        $header = '<script> changeStatusOnHover()</script>'
                . '<div id="chefsFollowFriendsHeader" style="font-size: 1.2em;"><img src="'.URL.'pictures/ENRI_CHEF.png" width="30">  Chefs You Follow/Friend</div>';
        ...
        for($looper = 0; $looper < count($chefsFollow); $looper++)
        {
            $user_id = $chefsFollow[$looper]['user_id'];
            $friend_user_id = $chefsFollow[$looper]['user_friend_id'];
            $output .= '<div class="FFHolderMain" style=" background-image: url('.$this->view->checkFFbckPic($chefsFollow[$looper]['cover_picture']).');">
                        <div class="PNTS"></div> 
                       <div class="friendPicMain">'.$this->view->checkUserPicture($chefsFollow[$looper]['picture'], 50, 50).'</div>
                      <div class="FriendNameMain"><a style="color:black;" href="'.URL.'profile/user/'.$this->encrypt($this->checkForFriendUser_id($this->id, $user_id, $friend_user_id)).'"><b>'. $chefsFollow[$looper]['FirstName']." ". $chefsFollow[$looper]['LastName'].' </b></a></div>
                      <div class="friendUsertTypeMain">'. $chefsFollow[$looper]['user_type'] .'<span><img class="FriendverifyImg" src="'.URL.'pictures/verify.png" width="12" height="12" alt=""></span> </div>
                     <div class="friendStatus" onclick="sendFriendShipToServer(\''.$this->getStatus($chefsFollow[$looper]['status'], $chefsFollow[$looper]['user_id'], $chefsFollow[$looper]['user_friend_id'], $userId).'\', \''.$chefsFollow[$looper]['user_id'].'\', \''.$chefsFollow[$looper]['user_friend_id'].'\', \''.$looper.'\')">'.$this->getStatus($chefsFollow[$looper]['status'], $chefsFollow[$looper]['user_id'], $chefsFollow[$looper]['user_friend_id'], $userId).'</div>
    ...
    function infinityFollowChef($page)
    {
         $chef = 'Chef';
